...
class IGenzSupabaseUser extends BaseAuthUser {
  IGenzSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> iGenzSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = IGenzSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
